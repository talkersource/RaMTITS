...
int main(int argc, char *argv[])
{
	...
	log("ident", "Ident message size = %d", req_size);
#endif
	...
	while (status != IDENT_STATUS_SHUTDOWN)
	{
		...
		case -1:
			 ...
			 log("ident", "ident: select failed: %s\n", strerror(errno));
...
void process_request(int toread)
{
	...
	for (i = 0 ; i < toread ;)
	{
		...
		default:
#if defined(DEBUG_IDENT_TOO)
			log("ident", "Ident_Server: Unknown message from client:"
							 " '%d'\n",
				req_buf[i]);
#endif /* DEBUG_IDENT_TOO */
			 break;
...
#if defined(DEBUG_IDENT_TOO)
	log("ident", "Server: Doing request - %d\n",
			  id_req->ident_id);
#endif /* DEBUG_IDENT_TOO */
...
{
#if defined(DEBUG_IDENT_TOO)
	log("ident", "Server: write_request()...\n");
#endif
	sprintf(scratch, "%d, %d\n",
...
#if defined(DEBUG_IDENT_TOO)
	log("ident", "Server: Sending request '%s'\n", scratch);
#endif /* DEBUG_IDENT_TOO */
}
...
void process_result(int slot)
{
	...
	*s++ = IDENT_SERVER_SEND_REPLY;
	...
	reply_text = s;
#if defined(DEBUG_IDENT_TOO)
	log("ident", "Server: Processing result...\n");
#endif /* DEBUG_IDENT_TOO */
...
#if defined(DEBUG_IDENT_TOO)
	*(t + 1) = '\0';
	log("ident", "    Sending reply '%s' of %d bytes\n", s, (t - scratch));
#endif /* DEBUG_IDENT_TOO */
